...
TASK: Find 3 flight options from {origin or 'a major city'} to {destination}.
{f'Travel period: {travel_time}' if travel_time else ''}

TOOL USAGE:
- Make exactly 1 search call with query: "flights {origin} to {destination} {travel_time or dates} prices {current_date[:4]}"
...
{"- Focus on budget-friendly options" if strict_budget else "- Mix of budget and premium options"}""",
    ...
    async def perform_task(self, query: str, context: Dict[str, Any] = {}) -> Dict[str, Any]:
        print(f"[{self.name}] Searching flights for: {query}")
        ...
        try:
            ...
            if isinstance(result, FlightList):
                flights_data = [f.model_dump() for f in result.flights]
            ...
            await self.shared_session.update_state("flights", flights_data)
